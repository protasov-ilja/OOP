...
rem провер€ем замену подстроки в 1 строке
%PROGRAM% one-string.txt %TEMP%\one-string.txt
if ERRORLEVEL 1 goto err
fc.exe %TEMP%\one-string.txt one-string.txt
if ERRORLEVEL 1 goto err

rem провер€ем замену подстроки в нескольких строках
